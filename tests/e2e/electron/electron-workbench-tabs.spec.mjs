  ...
  test(`Electron historical creation restart lifecycle: ${recoveryCase}`, {
    ...
    try {
      ...
      if (recoveryCase === "pending") {
        ...
        // Keep the opened-ack fault injected until the old process has really
        // exited. Otherwise the first renderer can confirm the receipt during
        // graceful close and make the restart recovery assertion vacuous.
        await closeStemmioGracefully(app.electronApp, app.page);
      }
      const beforeRestartReceipt = await repository.queryHistoryCreation({ target, operationId });
      expect(beforeRestartReceipt.openedAt).toBeNull();
      app = null;
