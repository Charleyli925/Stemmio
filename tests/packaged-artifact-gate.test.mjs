...
test("release commands use one automated artifact lane with full tests and packaged runtime verification", async () => {
  ...
  assert.equal(
    ...
    "npm run architecture:check && tsc --noEmit && npm run typecheck:source-receipt && npm run typecheck:document-source-operation && npm run typecheck:agent-credential-operation && npm run typecheck:browser-open",
  );
