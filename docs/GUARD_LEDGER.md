...
| Canvas upward scroll distance | Keep the latest visible reading position across same-document reload | presentation | upward movements over 400px previously restored an older position | document/frame restoration scope | record visible reading anchors; automatic comment alignment moves only the rail; explicit comment reveal is a new reading intent | delete distance heuristic; existing comment reveal sequence cancels pending callbacks on newer reading input or document reset |
| Missing project surface title | Labels are presentation, while project/document/version identity authorizes the target | presentation | previously refused rules/history navigation | existing target and Repository validation | use HTML until a valid Registry title arrives; refresh the same tab without changing focus | remove missing-title block; retain identity and display-safety checks |
| Replayed execution caption after a copy change | Stable event identity must not silently retarget a Request; fixed caption wording is presentation | presentation / authority split | previously blocked a verified current HTML on an old caption | Request/outbox event, turn, attempt, candidate and timestamp checks | retain previously stored caption while verifying the original event tuple; reject ownership/content collisions | degrade copy-only mismatch; keep identity and public-summary checks |
| Pending ordinary tab activation | Do not execute an abandoned intermediate existing-tab target | reversible | avoids an unnecessary read and activation wait | independent navigation operations remain queue barriers | latest adjacent target executes; replaced request settles as nonactivation | keep adjacent latest coalescing; preserve barriers and target checks |
...
| `PROJECT.md` during AI execution | Published Request rules remain immutable while current rules may advance | reversible | previously read-only for the whole run | Request input/PROJECT.md plus frozen hashes and execution allowlist | existing rules Session/Workflow saves next-round content normally | narrow to frozen or uncertain submission; retain HTML run lock and close/save guards |
...
| Current-draft presentation handoff | Keep the last visible canvas until the selected draft's exact Canvas can display | presentation | prior canvas inert and callback-free; incoming final canvas hidden until current generation and source SHA verify | ProjectWorkflow drain plus DocumentSession Canvas authority | exact verification reveals the new canvas; failure shows retry | no fixed delay, capture IPC, screenshot cover or Preview-button gate |
| Preview during background work | Opening the selected authoritative HTML does not depend on comment, Version or Edit Canvas background settlement | presentation | Preview stays enabled for matching document identity and ready HTML, except formal Review | document identity and source receipt | enter Preview from current HTML; Edit Canvas may finish separately | remove broad `interactionLocked` disable |
...
| Direct structure copy scope | Source identity, reference, resource, event, ancestor and mixed-content guards stay fail-closed; static script-free text `div` copies with safe inline descendants reuse the existing semantic kernel and in-place proof | presentation / post-validate | formatted text `div` uses the shared safe-inline subtree check | `direct-structure-policy` + semantic kernel + frozen source proof | reject before source receipt; accepted projection failures use Canvas recovery | degrade narrowly; retain all existing guards |
