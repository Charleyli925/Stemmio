...
- 一个 `projectId + documentId` 在标签栏中最多出现一次。切换文档标签必须经过现有
  ...
  都不常驻完整静态 iframe。切回命中的缓存标签时才按需挂载脚本禁用的临时交接面，同时立即进入正常
  Registry/OpenTarget 打开链路；不命中时不为制造展示缓存而推迟正式打开。临时交接面始终位于画布列，直接继承正式画布宽度，不跨越评论或 AI 侧栏，也不向用户展示交接时的宽度重排。静态面的 `onLoad` 直接报告精确候选 token 给唯一交接 owner；token 同时含缓存的 `tabId + sourceSha256` 和仅本次交接有效的导航身份，因而同一 Hash 的下一轮导航不会接收旧 iframe 的迟到回调。owner 保留完整已接受 token 作为当前物理展示实例；新候选在自己的完整 token 被接受前保持隐藏，即使两者恰好使用同一份缓存 HTML。`data-display-ready` 等属性仅供测试和诊断，不能被父组件观察为控制链。精确 Canvas 接管后交接面立即释放，快速反向切换最多短暂重叠旧面与新候选面。
