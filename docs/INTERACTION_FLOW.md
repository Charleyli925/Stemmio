...
“更多 → 找回此前的稿件…”是当前稿的迁移/保全恢复入口，在历史页保留但禁用，并说明应先打开当前稿。打开后必须显示正在读取、明确空状态或错误，不能无反馈。有条目时列出旧工作稿或此前当前稿，展示时间、基于 Vn 和“含评论”，本次不预览 HTML。用户明确选择“恢复为当前稿”才创建下一份 `recovery-copy` Version，恢复 HTML、评论草稿及附件，并先保留当前稿；不回滚任何旧历史或项目长期规则。
