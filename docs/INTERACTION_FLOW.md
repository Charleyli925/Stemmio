...
- 标签栏采用 roving focus：左右方向键循环移动并激活，Home/End 到首尾；安全切换失败时焦点和活动
  ...
  活动开始页仍作为目标文档的同一标签原位打开。等待切入非活动标签时，从准备状态起仅目标标签显示淡靛紫底色与稳定的紫色细底线；
...
- 第二层工具栏只承担模式、审阅视图、文件动作和 AI 入口。顶栏不提供“项目”按钮、项目管理入口，也不显示保存状态点、写入中、冲突或失败文字；保存/冲突事实继续由 Document/Project 工作流、标签状态点和全局错误提示承载。刷新与更多图标保持 20px 视觉盒、30px 点击区，顺序为“刷新 → 上轮处理（有时）→ AI 助手 → 更多”。
- 更多菜单在当前稿和历史页保持同一套操作顺序；不适用于当前页的操作保留但禁用，并就近说明实际作用对象。“在 Finder 中显示工作文件”仍只作用于当前稿；默认浏览器动作则打开用户此刻所见的精确对象：当前页为安全写回后的工作文件，历史页为已核验的不可变 Version，文案分别为“在浏览器中打开工作文件”和“在浏览器中打开此版本”。纯浏览器预览没有本地文件权限，因此只展示禁用态。失败显示可恢复的全局错误，不通过顶栏重复呈现。
- Review 中顶栏刷新只“刷新本页面”：重载两侧隔离 Runtime，并在同一 session 身份核对后恢复当前 Tab/折叠、聚焦、滚动与缩放，不读取磁盘新源。更多菜单中的“从磁盘重新载入 HTML”在待决定 Review 期间保持可见但禁用，并就近说明必须先采用或不用本次 AI 修改；两条刷新语义不得互相替代。
...
- 已有文字会话内再次双击同一宿主时保留浏览器选词，不重建 Controller。双击另一个文字宿主时先收口旧会话；收口失败不启动新会话。IME 期间沿既有延迟命令队列等待组合结束，再按当前源码身份重新解析点击意图，不能复用旧 DOM 对象。
- 双击 B 收口 A 时，如果 A 已成功进入 authoritative source 但必须换 Frame，B 作为更新的用户意图按 source receipt、Hash、Canvas generation 和可重解析的目标保留；新 Active 建立后重新验证并进入 B，不沿用旧 DOM 或屏幕坐标。等待期间的 C 选择、清空或外部输入焦点以更高意图代次取消 B，旧意图不得在 Candidate promotion 后抢回焦点。
...
历史“导出此版本…”复制所见快照。更多菜单保留但禁用“在 Finder 中显示工作文件”；“在浏览器中打开此版本”可用，且只打开所见不可变 Version 的经验证文件，不读取、写入或冒充当前稿。隐藏快照仍不作为可编辑用户文件发布。
...
当前“找回此前的稿件”只表示从当前已支持的保留稿创建新版本。它不扫描或收尾旧工作副本，也不提供迁移入口；没有当前保留稿时直接显示空状态。

保留稿对话框的加载与恢复回调必须绑定当次打开会话。用户关闭后重新打开时，上一次的迟到成功、失败或 `finally` 不能关闭新对话框、改写新列表、清理新恢复状态或显示过期错误。
