...
当前稿的预览实例在返回编辑且编辑可操作后，最多再隐藏保留 3 秒；同一文档、同一源码与画布代次、同一已验证实例在这段时间内再次进入预览时，先向 Main 核实资源会话仍有效，再直接复用，不重新启动作者脚本。核实失败或会话已过期、被淘汰时重新创建；旧显示回执不能替新会话确认就绪。隐藏实例仍可能运行脚本并占用网络、CPU 与内存，因此到期即释放；源码或目标身份变化、切到其他标签、关闭、失败及明确重新加载会撤销旧回执的复用资格。任何时候都只有当前 Edit 画布拥有写入入口，预览不成为源码来源。
