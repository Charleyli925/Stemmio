...
品牌头像及名称，连续事实合为紧凑记录，过程默认折叠为一行最新非空公开段落预览；预览只整理空白，
...
展开历史或折叠高度变化均保留用户操作的触发器锚点，不抢滚动；上滚后显示“回到最新”，只有阅读期间
收到新内容才显示“有新进展”，点击后恢复底部跟随。侧栏重挂载按打开标签
...
Stemmio 校验与准备 Review。相邻同主体同文案过程合并显示次数，事实记录仍保留；历史过程默认折叠，
