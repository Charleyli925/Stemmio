...
- 一个 `projectId + documentId` 在标签栏中最多出现一次。切换文档标签必须经过现有
  ...
  都不常驻完整静态 iframe。普通当前稿切换仍走 Registry/OpenTarget 安全打开链路；点击 B 后 A 的真实画布以原尺寸留在原位但成为不可编辑、不可保存的只读画面，B 的正式画布在同一位置准备。B 的当前代次与源码 Hash 经 Canvas 核对，且需要脚本的页面完成同一源码和代次的可见运行态或明确回退后，A 一次退场、B 一次显示；无脚本页面在核对完成后显示，失败则撤下旧画面并显示错误和重试。不会增加截图遮罩、固定等待或长期第二编辑器。HTML 数据缓存仅用于有界投影和阅读状态；原静态 HTML 交接实现只在显式 E2E 诊断开关下启用，不参与普通切换。
