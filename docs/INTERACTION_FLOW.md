...
变化事实由分析器按 `text` 与 `structure` 记录，筛选只改变当前可见集合，不产生顺序计数或导航队列。页面上的变化标记是区域内明细事实和定位入口；已有变化但当前筛选没有匹配时保留筛选并显示空态；全部事实为空时不展示变化筛选组。两侧页头共用白底，修改前保持中性标识，AI 修改后使用紫色顶边与紫点，让横向滚动后仍能立刻分辨哪一侧是新版本。
