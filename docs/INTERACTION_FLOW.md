...
重新分组。Agent 的服务名、计时与已接收 KB 归属于同一条当前 Agent 消息的头部，不另建状态段或一条常驻的
Stemmio 状态压在其下。Agent 结束后，后续校验与 Candidate 事实再按发生顺序追加。停止操作仍固定在底部。侧栏与画布只保留 1px 视觉分隔线，18px 透明拖拽命中区不另画粗条。
