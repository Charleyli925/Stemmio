...
- 项目身份、Registry、长期规则、Version、Candidate 和历史 HTML 仍由对应事实与工作流持有。当前稿、长期规则和历史都是项目作用域的独立标签页，标签以“项目名 · 页面类型”呈现；项目展示标题缺失或为空白时，规则和历史标签先使用“HTML”占位，Registry 补齐真实标题后只更新已有标签的显示，不改变身份、焦点或当前稿。每个项目最多各保留一个当前稿、规则和历史标签。重复打开同项目历史只更新该标签的 Vn，不按版本无限增加标签。跨项目点击规则或历史时，导航先安全收口原项目并加载目标项目，只在目标页面就绪后提交活动标签，不先闪现目标项目当前稿。目标规则的读取结果先暂存，标签与规则编辑会话再同步提交；读取或标签提交失败时，原标签、原内容、重试身份和保存目标保持一致。编辑、还原、保存与重试还会核对可见规则标签的项目/文档身份。规则页支持基础 Markdown、700ms 自动保存和 `⌘S` 立即保存，保存状态只在编辑器原位安静显示。内部保存文件名与协议保持不变，但不向用户展示。AI 任务开始时由 Request freeze 读取并冻结规则快照，任务期间编辑只影响下一次任务；恢复或切换 HTML 历史版本不回滚该文件。
