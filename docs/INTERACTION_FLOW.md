...
4. 普通编辑自动保存；“保存到历史版本”主动留下本地历史。修改前稿件、AI冻结输入和所有旧历史都可保留，同一当前稿继续编辑。
...
- 更多菜单提供“保存到历史版本”，并说明“保留此刻的内容，继续编辑当前稿”；其他位置不重复提供独立建版入口。打开或切换 HTML 统一走左侧栏、开始页或外部打开的安全项目流程。
...
“更多 → 保存到历史版本”是独立动作：先收口并保存当前完整 HTML，再验证身份、Hash 和项目空闲条件，保存下一不可变 Version并继续编辑同一当前稿。当前评论附件保留。无内容变化时提示“当前内容已保存在历史版本中”，不增加编号；未知提交结果只查询/重试同一操作。
