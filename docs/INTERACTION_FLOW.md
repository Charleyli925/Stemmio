...
4. 普通编辑自动保存；“保存为历史版本”主动留下本地历史。修改前稿件、AI冻结输入和所有旧历史都可保留，同一当前稿继续编辑。
5. 编辑画布上：单击选择，双击改字；需要点页面自己的按钮或脚本交互时切到“预览”。部分 Tab 等源码可证明的切换在编辑里也能做，不必为它们切预览。
6. 动态内容无法运行而静态源码页面仍可编辑时保持安静，不显示常驻警告；可重试的临时失败在“更多”菜单提供“重新加载动态内容”，先保护当前修改再重试，不支持的程序不提供无效重试。动态与静态 Candidate 都失败时保留上一张可用画面为只读，顶部提示“页面暂时无法编辑”，持续提供重新加载和导出。“更多”菜单的普通“刷新”在画布核对同一源 Hash 并释放编辑锁后安静完成，不显示成功提示；失败或从只读故障恢复时仍提供对应的全局反馈。即使动态准备失败，新静态画布核对成功也必须解除旧画布的只读降级。Candidate 准备期间保留当前图表与滚动位置，文字输入和中文组词尚未结束时不切换 iframe。
...
- 顶部是浏览器式双层骨架：第一层是全局标签栏，第二层是编辑、预览与审阅共用的
  ...
  收起时只在系统红黄绿按钮旁保留唯一的侧栏开关。开合期间开关位置固定，标签与侧栏同步短促移动，
  不应让“开始”标签闪到窗口左缘；关闭动态效果或系统偏好减少动态时立即切换。
  标签状态点只投影正常、AI 处理中、待审阅与错误；
...
- 一个 `projectId + documentId` 在标签栏中最多出现一次。切换文档标签必须经过现有
  ...
  在后台继续核对，未完成前只锁住编辑能力，不用空白页遮住已到达的 HTML。后台核对失败时保留已打开页面并把
  标签标成错误，不把“能看到页面”伪装成打开失败。全应用仍只有一套
  `WorkspaceController` 和一个挂载的业务文档。标签可在 32 MiB / 20 项预算内保留精确 HTML 数据，但普通活动或非活动标签
  都不常驻完整静态 iframe。普通当前稿切换仍走 Registry/OpenTarget 安全打开链路；点击 B 后 A 的真实画布以原尺寸留在原位但成为不可编辑、不可保存的只读画面，B 的正式画布在同一位置准备。B 的当前代次与最新源码 Hash 经 Canvas 核对，再核对当前物理帧中的运行实例或无脚本静态结果，或者进入明确静态降级；普通安全编辑已在该帧验证原位投影时，允许运行实例继续使用原启动源码 Hash，不重跑作者脚本。核对完成后 A 一次退场、B 一次显示；失败则撤下旧画面并显示错误和重试。不会增加截图遮罩、固定等待或长期第二编辑器。HTML 数据缓存仅用于有界数据和阅读状态，不挂载文档或 iframe。
  展示交接以用户当前目标、实际留在屏幕上的实例、目标缺少的证据和能否正式揭示为一次决定；B 准备期间保留目标标签的加载下划线，顶部正式选中态与模式按钮仍对应屏幕上的 A，A 已失去全部输入能力。B 真正显示时，正式选中态、模式按钮和画布一起切到 B。Preview 从活动画面转为交接中的旧画面时保留同一个物理 iframe 与运行会话，不因角色变化重新挂载。旧请求被后续点击替代后不能再揭示；失败和关闭会释放旧画面，重试从新尝试取得证据。
  滚动、模式与有界页面上下文独立于 HTML 数据保留，只在标签、项目、文档与源码 Hash 全部一致时恢复；HTML 淘汰不丢阅读位置，新修订不恢复旧源码目标上下文。
  启动恢复直接开始正式标签激活，不先读取缓存数据或预热其他非活动标签。非活动标签绝不保留
  contenteditable、Selection、IME、编辑观察器或保存权威。切换完成后活动文档只挂载一个
  `HtmlCanvasEditor`；切换瞬间可短暂并存已失去全部写入回调的旧画布和新正式画布。脚本刷新只在编辑器内部短暂 A/B Candidate 完成，不得在 Runtime
  preparing 时整页换成独立静态文档，也不得让数据缓存挡住文字输入。
  ...
  切换中的当前稿目标标签保留紫色下划线，直到目标画布真正可显示；候选编辑画布以最终尺寸
  布局并保持可绘制，在交接前透明且不可交互，避免揭示时才开始布局脚本内容。
  从活动开始页继续项目、处理待办或新建项目时，该开始标签原位变成目标文档；只有用户明确点击「+」
  创建的其他开始标签会保留。若目标文档已存在，则移除当前空白标签并聚焦已有标签。从活动文档切到独立开始标签再关闭开始标签时，若原文档仍由同一 Controller 和 Runtime 持有，直接恢复原编辑画布，不重复打开文档或准备 Runtime。
  当一个普通已有标签正在安全切换时，后续相邻的普通已有标签点击只保留最后一个目标；已开始的切换照常完成，被替代点击以成功但未激活的结果收口且不显示错误提示。打开规则或历史、创建、关闭及外部打开等独立动作形成队列屏障，不被合并或越过。
...
- 标签栏采用 roving focus：左右方向键循环移动并激活，Home/End 到首尾；安全切换失败时焦点和活动
  ...
  活动开始页仍作为目标文档的同一标签原位打开。等待切入非活动标签时，从准备状态起仅目标标签显示淡靛紫底色与更浅的紫色细底线；底线以轻微明暗变化持续表示正在打开；
  原标签同时撤下视觉选中态，但 `aria-selected` 与画布权威仍指向原标签，直到目标安全提交。等待目标的辅助名称说明“正在打开”；
  提交后目标底线持续到对应页面可显示，失败或目标被替代时恢复实际活动标签的视觉选中态。
  在已选中标签内切换历史版本时保留该标签原有名称与选中样式，
  当前文档的编辑、保存完成和新增评论产生短暂活动反馈；写入中或 AI 正在处理时持续显示同一条活动线。首次载入、后台文档的活动、等待审阅决定、失败或取消终态不冒充持续处理。切换目标清除旧活动；减少动态效果的系统或应用设置下底线保持静态可见。

### 工具栏、项目上下文与设置

- 编辑、预览、审阅、设置与两类右栏共用 [设计语言](DESIGN_LANGUAGE.md#3-视觉基础) 的白色底面与文字层级；模式入口使用细下划线选中态。视觉切换不改变各模式权限、评论定位、导航、设置或 AI 决策流程。
- 左侧栏是全局项目上下文，固定分为标题栏、品牌区、可滚动项目列表和底部固定操作区。品牌区使用 Stemmio Logo 和“Stemmio”名称。项目列表只有一个“项目”分组：当前项目与 Registry 项目按真实 `projectId` 去重后混合展示，并按权威内容更新时间倒序排列；打开项目不会改变排序。每个项目独立维护展开状态，多个项目可以同时展开，展开或收起不会切换当前项目，也不会读取 HTML 正文、评论或附件。每个展开项目依次提供长期规则、当前稿和默认折叠的历史版本；三类入口通过既有安全导航打开。品牌区打开 About，底部齿轮在标签栏中打开设置。
- 第二层工具栏只承担模式、审阅视图、文件动作和 AI 入口。顶栏不提供“项目”按钮、项目管理入口，也不显示保存状态点、写入中、冲突或失败文字；保存/冲突事实继续由 Document/Project 工作流、标签状态点和全局错误提示承载。更多图标保持既有点击区；次序为“上轮处理（有时）→ AI 助手 → 更多”。刷新收进更多菜单。
- 更多菜单在当前稿和历史页保持同一套操作顺序；不适用于当前页的操作保留但禁用，并就近说明实际作用对象。“在 Finder 中显示”仍只作用于当前稿；默认浏览器动作则打开用户此刻所见的精确对象：当前页为安全写回后的工作文件，历史页为已核验的不可变 Version，文案分别为“在浏览器中打开”和“在浏览器中打开”。纯浏览器预览没有本地文件权限，因此只展示禁用态。失败显示可恢复的全局错误，不通过顶栏重复呈现。
- 更多菜单只有一个“刷新”：通过既有安全重载工作流读取已保存的当前稿，保留编辑或预览模式；不另提供只重绘 iframe 的常规入口。待决定 Review 期间禁用并说明“请先采用或不用这次 AI 修改，再刷新”。历史页禁用；已有未保存/冲突保护不变。菜单以透明点击退出面覆盖 HTML iframe 与外层界面，菜单以外的第一次点击仅收起菜单，Escape 收起并归还焦点；菜单内点击不误关闭禁用项。
- 项目身份、Registry、长期规则、Version、Candidate 和历史 HTML 仍由对应事实与工作流持有。当前稿、长期规则和历史都是项目作用域的独立标签页，标签以“项目名 · 页面类型”呈现；项目展示标题缺失或为空白时，规则和历史标签先使用“HTML”占位，Registry 补齐真实标题后只更新已有标签的显示，不改变身份、焦点或当前稿。每个项目最多各保留一个当前稿、规则和历史标签。重复打开同项目历史只更新该标签的 Vn，不按版本无限增加标签。跨项目点击规则或历史时，导航先安全收口原项目并加载目标项目，只在目标页面就绪后提交活动标签，不先闪现目标项目当前稿。目标规则的读取结果先暂存，标签与规则编辑会话再同步提交；读取或标签提交失败时，原标签、原内容、重试身份和保存目标保持一致。编辑、还原、保存与重试还会核对可见规则标签的项目/文档身份。规则页支持基础 Markdown、700ms 自动保存和 `⌘S` 立即保存，保存状态只在编辑器原位安静显示。内部保存文件名与协议保持不变，但不向用户展示。发送前继续等待规则输入与保存完成；提交冻结和结果未确认期间，规则暂时只读。Request 发布并完成提交协调后，本轮只读取冻结规则，规则页可继续编辑并自动保存下一轮内容，HTML 当前稿仍保持原有 AI 编辑锁；恢复或切换 HTML 历史版本不回滚该文件。
...
- 更多菜单始终展示核心操作，不适用项使用 `aria-disabled`、可聚焦的菜单项和就地原因说明；方向键遍历包括禁用项，`Enter` 不执行禁用动作，`Escape` 返回触发器，`Tab` / `Shift+Tab` 一次离开菜单并继续页面的正常焦点顺序。
...
- 软件更新在发现可用版本但尚未下载时继续按原有首次 5 秒、后续 4 小时节奏检查，也允许通过设置页的“再次检查”手动刷新，下载仍是主操作。检查期间显示检查中；下载等待本次检查确认候选，失败不冒充最新版，明确无更新时清除旧候选。已经下载或开始下载的制品保持固定；不自动下载或安装。
- Agent 接入的连接验证、凭据保存和默认项提交是三个独立结果。设置与连接卡片依据凭据操作类型、状态和错误码选择恢复动作，不读取提示文案：启动读取失败要求重新连接，当前连接的保存失败仅重试保存，未知删除核对原操作；鉴权、模型与地址错误分别落在对应字段，未识别错误留在表单。保存回执未知时先按原 `operationId` 查询；单独重试保存不得重新连接或重复提交默认项。后发的连接、清除或默认意图围栏迟到的旧结果，清除后的旧写回放不得复活 Key。
- About 只介绍源页产品、版本、架构、公开下载/支持入口和用户声明；它绝不暴露或打开私有源码仓库，不触发 Agent 检查，也不承载登录、连接状态或软件更新。点击品牌或 macOS“关于源页”都进入 About；About/设置均支持遮罩或 Escape（设置为标签页关闭）、焦点约束和关闭后焦点回归。
- 审阅工具保留“变化 N 处”目录、双页/修改前/修改后、同步或独立滚动和缩放；不再提供全部/文字/元素筛选。
  上下文可见度移到“设置 → 常规”，变化聚焦与评论聚焦分别保存，默认 25% / 15%。共享顶栏提供
  可展开的“变化 N 处”目录；页边只保留低噪声位置提示，不提供上一处、下一处或重复计数导航。
  第一次进入只自动定位第一处变化一次，不激活遮罩或边框。
- 更多菜单提供“保存为历史版本”，并说明“保留此刻的内容，继续编辑当前稿”；其他位置不重复提供独立建版入口。打开或切换 HTML 统一走左侧栏、开始页或外部打开的安全项目流程。
...
当前稿从编辑进入预览时，编辑画布保持切换前的尺寸并留在预览下方，直到新预览可显示；
预览就绪后仍保留该画布的绘制状态，以便返回编辑时直接接续，不让图表因隐藏、重排而闪动。
下层画布在预览期间不可交互，也不取得预览页面的权限。
预览的 iframe `load` 不单独代表可见页面就绪；注入的受限预览脚本在首个内容绘制后回执，宿主再给合成层绘制机会，空白页面或脚本受限时采用有界回退。
每次预览尝试独立记录结果：收到当前 iframe 的首个内容绘制证据为“已验证显示”；在限定时间内仍无绘制证据但身份正确为“降级显示”，不得把它称为已经确认的空白页面；无法建立页面或明确加载失败为“失败”；被后续模式、标签或重试替代为“取消”。降级显示仍结束 opening，正常显示，无需新增常驻提示；仅失败提供错误与重试。会话、实例代次和请求身份必须先核对，再接受绘制回执或限时回退。
诊断按每次尝试分别记录编辑 iframe 创建、Runtime 准备及执行、实际作者脚本尝试数、Preview 会话创建、画面交接及旧实例释放，并附目标类型、动作原因、阶段耗时和结果。编辑画布用活动代号关联物理帧、候选和退场；Runtime 准备用内部尝试代号，获批后以会话 ID 关联执行、作者脚本和释放；Preview 用尝试代号关联创建、显示和取消。不记录 HTML 正文或本机路径。诊断标记只用于解释行为，不构成新的显示条件。
目标预览明确失败时，旧文档的预览画面立即退场，当前目标显示错误和“重试打开”；重试创建新会话，旧 iframe 的加载或失败回调不能改变新会话结果。
历史版本的独立预览会话失败而进入只读静态降级时，静态内容和降级说明持续显示，直到用户主动刷新重试；静态帧加载成功不清除会话失败状态。
从预览返回编辑时，同一文档的已加载预览留在前景，直到编辑运行态完成并经过短暂绘制交接；跨文档的旧编辑画面在目标预览就绪时退役，不得在目标编辑准备期间重新出现。
当前稿的预览实例在返回编辑且编辑可操作后，最多再隐藏保留 3 秒；同一文档、同一已接受编辑修订与画布代次、同一已验证实例在这段时间内再次进入预览时，先向 Main 核实资源会话仍有效，再直接复用，不重新启动作者脚本。编辑修订一经接受就撤销旧实例资格，不等待自动保存推进磁盘 Hash；保存回执随后到达也不应让内容未变的新实例再次重建。预览显示以当前工作 HTML 已确认的 Hash 核对，尚未取得该 Hash 时仍只接受当前物理实例的显示回执。核实失败或会话已过期、被淘汰时重新创建；旧显示回执不能替新会话确认就绪。隐藏实例仍可能运行脚本并占用网络、CPU 与内存，因此到期即释放；源码或目标身份变化、切到其他标签、关闭、失败及明确重新加载会撤销旧回执的复用资格。任何时候都只有当前 Edit 画布拥有写入入口，预览不成为源码来源。
直接进入已保存的 Preview 模式时，暂不启动该文档的 Edit Runtime 准备；首次真正进入 Edit 再按现有源码与画布代次校验准备，预览本身的加载及失败出口不依赖提前准备 Edit。被替代的导航仍遵守当前目标身份与会话取消规则。
纯浏览器的内存文档没有桌面 DocumentSession/Runtime 回执，返回编辑时直接显示已保留的编辑画布，不让预览层无限遮挡编辑。
跨标签进入已保存的预览模式时，先将模式下划线移到“预览”，预览加载期间用按钮原位的
转圈图标代替眼睛；新页面就绪后再交接画布并恢复眼睛图标，不另显示浮动的加载文案。
...
同一文档重新载入时保留最近可见的阅读位置；向上、向下滚动都更新阅读锚点，
不根据移动距离忽略用户操作。评论栏自身的对齐只移动评论栏；用户明确请求
定位评论时，新的页面位置可以成为后续恢复的阅读位置。评论提交等待或定位帧尚未执行时，
画布及共享阅读区的新滚轮、指针或键盘操作会撤销旧定位；文档切换同样使旧请求失效。
用户随后主动点击评论仍可再次定位，不让迟到回调覆盖较新的阅读意图。
评论定位执行时使用当前标题栏的实际尺寸，避免保存评论后的标题栏收缩通知迟到时使用旧落点；后续布局通知本身不得再次滚动页面。
...
直接入口承认独立的 `p`、`h1`—`h6`、简单 `li`、直接承载行内内容的
`blockquote`，以及无脚本静态文档中含有非空文字的 `div`；这些文字块的后代只能是已有安全行内格式（如 `span`、`strong`、`em`）与 `br`，不能包含嵌套块。
嵌套块/列表、控件、媒体、表格、图表、
SVG/Math、脚本/资源、自定义元素、注释、作者 ID 或需要引用重写的内容都会拒绝。
静态 `div` 文档只要任意位置存在作者 Script、事件属性或按 WHATWG URL 解析为
`javascript:`/`vbscript:` 的导航/载入 URL 属性（如 `href`、`xlink:href`、`src`、`action`、`formaction`、`object[data]`），就不显示复制入口，命令层也统一拒绝；运行时生成内容、不透明运行表面或程序的容器也不显示复制入口，命令层统一拒绝；
...
直接结构工具的可见准入先读取当前源码目标和可执行方向，再做必要的 Runtime 子树证明；
没有合法的上移/下移方向时不做完整 Runtime 探测。复制、移动和删除的拒绝提示只保留
三类用户语义：对象不在直接编辑支持范围、不能跨组或跨位置移动、当前页面内容发生变化
暂时不能执行；内部 reason code 仍写入诊断，不向用户展开实现细节。More 菜单中的“从磁盘
重新载入 HTML”属于权威替换：完成同字节 reload 后可以继续 Native Edit、保存，正常关闭
并重新打开原项目时恢复修改后的源码和 Stable ID；它不等同于普通的页面刷新。
...
```text
单击只建立当前结构对象的选择，不因此进入文字编辑或显示“编辑中”
→ 双击实际文字 glyph，或双击已受管 `<br>` 对应的空行行首光标落点，按点击位置解析对应的最小安全文字宿主并把光标放到该位置（第一次双击不选中整词）；普通块级 padding 仍只保留结构选择
→ 空行命中同时要求指针下最上层可见命中属于同一源码宿主，且是未冻结、非控件的 authored `<br>`；被上层元素遮挡的底层空行不能只凭 Range 几何成为候选，入口和 Controller 共用同一个精确 `<br>` 身份
→ 透明行内节点归入同一文字宿主，解析不跨块级或控件边界；找不到安全文字宿主时安静保留结构选择
→ 工具栏“编辑”和 Enter 没有点击坐标，只在当前结构对象能唯一解析为一个最小文字宿主时进入；包含多个独立文字宿主时安静保留结构选择，不猜测第一段，也不把复合容器整体设为编辑区
...
- 行内透明节点只在同一文字宿主内归并；块级盒或控件边界停止提升。复杂父容器只有在点击其直属文字且自身就是最小安全文字宿主时才作为岛：非行内后代保持冻结原子，直属文字和行内标签在同一宿主里编辑。
- 已有文字会话内再次双击同一宿主时保留浏览器选词，不重建 Controller。切换另一个文字宿主时，第一次外部按下先收口旧会话，随后的独立点击才选择、独立双击才进入新宿主；收口失败不启动新会话。IME 期间沿既有延迟命令队列等待组合结束，再按当前源码身份重新解析点击意图，不能复用旧 DOM 对象。
- A 已收口后，用户另行发出进入 B 的意图时，如果 A 已成功进入 authoritative source 但仍须换 Frame，B 作为更新的用户意图按 source receipt、Hash、Canvas generation 和可重解析的目标保留；新 Active 建立后重新验证并进入 B，不沿用旧 DOM 或屏幕坐标。等待期间的 C 选择、清空或外部输入焦点以更高意图代次取消 B，旧意图不得在 Candidate promotion 后抢回焦点。仅用于退出 A 的第一次外部点击不创建进入 B 的意图。
- 在活跃文字岛内双击 SVG、Canvas、表单控件或其他冻结原子，属于“收口文字会话后切换结构选区”的同一事务。非 IME 状态先完成旧岛 checkpoint，失败则保留旧会话和旧选区；IME 状态继续使用延迟队列，组合结束后按 Stable ID 和当前 Document 重解析原子。不得先切换结构选区再丢弃尚未 checkpoint 的输入，也不得跨 Frame reload 沿用旧 DOM。
...
- 整个岛在一次会话内保持打开。连续输入、删除、粘贴、Enter、组词完成、常用文字样式、自动保存、⌘S、保存评论/附件和导出只做软 checkpoint：完整 Working HTML、Stable ID 和恢复证据进入同一保存队列，但保留同一 iframe、`contenteditable`、Selection、Caret 与 Focus，不启动 Runtime candidate。静态降级页中的已持久化 checkpoint 同时成为下一次显式 Retry 的唯一源码输入，但不会因输入本身运行 Script。只有点击无关页面区域、Escape 或硬离开才结束原生编辑。在编辑宿主之外单击其他 HTML 元素、控件或空白时，该次手势只提交并结束编辑、清空选择；下一次单击才选择其他对象，退出手势不计入双击进入编辑。宿主内原生选择、工具条和共享滚动条继续按既有规则工作；IME 等待组合结束后完成退出，提交失败保留会话。项目/标签/页面切换、进入历史、Preview/Review、AI 提交和 App 关闭使用硬边界；若目标已离开 Edit Canvas，只收口完整源码并移除编辑主机，不为离开前“刷新一下”重建 Runtime。
...
“更多 → 保存为历史版本”是独立动作：先收口并保存当前完整 HTML，再验证身份、Hash 和项目空闲条件，保存下一不可变 Version并继续编辑同一当前稿。当前评论附件保留。无内容变化时提示“当前内容已保存在历史版本中”，不增加编号；未知提交结果只查询/重试同一操作。

“导出当前 HTML…”原样复制收口后的完整当前源码，包括 Stable ID；“导出此版本…”复制所见不可变历史。两者不清洗源码、不序列化 Runtime，不导出评论、附件或项目记录。

点击“导出当前 HTML…”或使用常规导出快捷键，先显示对话框，默认勾选“同时保存到历史版本”，说明“在项目历史中保留此刻的内容，之后可随时查看。你可以继续编辑当前稿。”用户可取消勾选，点击“选择保存位置…”后才打开系统保存窗口。中文组词期间等待组合结束后再打开选项；取消或确认选项时归还原入口焦点，快捷键从 HTML 编辑宿主进入时归还该宿主。版本暂不可保存时显示禁用原因，仍允许单独导出。更多菜单不再有独立复选项。勾选后先成功导出，再用该次相同字节/Hash建版。无变化不重复建版，任一对话框取消或文件写入失败不建版；导出成功但建版失败时保留“文件已导出”事实，只查询/重试建版。历史直接导出所见版本，不提供此选项。
...
- **预览外部版本** 只读展示磁盘当前 HTML，不写入任何持久化状态。预览携带精确的项目身份、字节与 Hash 回执；用户可“返回我的编辑”或“接受此版本（丢弃我的编辑）”。预览后磁盘再变化时，原回执过期，必须重新预览，不能把同意扩大到未展示的字节。
- **采用磁盘版本** 会二次确认后清空 Working Copy 锁状态、采用已确认的精确磁盘 Hash 并把画布换成已接纳内容，不把内存编辑写回 HTML。这是冲突死锁的确定恢复路径，避免手动删除 `.stemmio/`。预览中的“接受此版本”走同一应用命令；Repository 在修改状态前再核对 Hash，并把已接纳字节直接交给投影，中间不二次读盘。
- 重载、冲突接受和画布修复分别回报“已授权 / 源码已接纳 / 页面已恢复”。源码已接纳但画布失败时不回滚或再读盘；“重试恢复”先加入现有 Document 保存单飞，然后只重建当前已接纳投影。
- 授权阶段明确区分 `not-required` 与 `accepted`。如果 Repository 已完成外部源接纳时用户已切页，结果保留 `source: accepted`、标记 `page: not-current`，不向新页投影或解锁。
- AI 冲突的“保留外部 HTML”由 `RunWorkflow` 先取得 Document 预览，本地实算 Hash 并与冲突 Hash 精确匹配，再交给同一 `DocumentWorkflow` 接纳命令；Workbench 只呈现最终结果，不串联第二段权威变更。
...
- 检测中：只显示“检测中”，不显示按钮；
- 已连接：只显示“已连接”，不显示重新检测；
- 未安装：显示“未安装”和“安装 Qoder CLI”或“安装 Codex 连接组件”；安装中显示“正在安装…”与可点击的“取消”，点击后原位显示“正在取消…”，完成后回到“未安装”；失败后原位显示稳定中文原因并可重试；
...
阶段、Agent 身份与时间、稳定的 `visibleTextUpdates`、截断标记、事件数和可恢复错误。Qoder ACP 与 Codex
ACP 都只把 Agent 明确发送给用户的文本归一为 `visible-text`；推理、工具参数、文件内容、
系统提示、异常堆栈和 finalizer 输出一律不进入侧栏。先按消息组装原始碎片、再脱敏；公开合同只保留有稳定 ID 的有序消息块，最多 65536 个 UTF-16 单元（含展示分隔与脱敏替换），不并行转发累计 `visibleText`。复制全文和封存摘要只在各自边界以 `\n\n` 连接这些已脱敏块；诊断事件上限不裁掉正文。真实截断必须明确标记。

公开活动只读取受限 Host / Runtime 的固定事件：冻结资料读取、独立结果写入、收到响应、
生成开始、响应结束、HTML 完整性检查、准备审阅与停止请求。不显示命令、路径、参数、
文件正文或工具输出，不增加打开文件/执行命令入口。HTTP 不显示 ACP 文件工具活动；
剪贴板交接没有外部执行观察记录。检查完整性不代表视觉验证，写入结果不代表采用。
公开文本以首次出现的 `firstSequence` 保持位置；活动与说明依该顺序穿插，同类相邻活动
仅在没有公开说明、失败、停止或完成边界时合并，身份沿用第一条事件，不显示文件数或成功勾。
活动明细最多保留 80 条；旧说明块合并后，省略落在合并区间内、无法准确穿插定位的活动，保留完整说明文字。省略时如实提示；它只保留在本次 Bridge 内存中，不扩充 Conversation
持久化协议。重启只恢复已有阶段和结果事实，不重建活动列表。复制仍只包含已脱敏公开正文。
...
受管执行时，对话侧栏显示历史事实和一条当前 Agent 消息：头像旁只放服务名及
`用时 · 已接收 KB`的小字元数据，正文只显示实际存在的 Agent 公开文本。尚无公开文本时只显示一个安静的三点指示；
一旦有公开文本就去掉该指示，不再叠加“正在生成 / 正在接收 / 完整校验后可查看”等重复说明。复制只包含公开正文；HTTP 整页 HTML 不进入聊天。
...
只对不随秒数或字节变化的简短处理状态使用礼貌播报。历史与流式公开文本均可复制；当前执行消息不再叠加墙钟时间。
...
资料刷新延续当前源码回执，不更换已验证的 Canvas generation。无需重开项目即可查看新版本、继续编辑保存及发起下一轮。
采用正在提交或结果待确认时，不展示冲突决策的旧按钮。受管停止未确认时显示禁用的“正在停止”，
失败后继续保留本轮已有的公开说明。
...
审阅显示由五组正交状态组成，任何控件只能写自己的状态：

- `pageView`：`双页 / 左页·修改前 / 右页·修改后`，任一时刻恰好选中一个。页面按钮只改变可见页面，不改变上下文可见度或当前定位；必须点击明确的“双页”返回双页，不使用隐藏手势。
...
进入审阅的默认组合是 `双页 + 同步滚动 + 适应画布`，变化聚焦上下文默认 25%，评论聚焦默认 15%。初始视觉状态是总览：`activeFocusGroupId = null`。总览保留全部精确字符点/删除线、页边位置提示与“变化 N 处”目录，不画边框、不生成遮罩，也不虚化页面。“变化 N 处”紧跟顶部“审阅”图标，不再占用画布内部横条。两页首次 ready 后可以尝试把第一处变化作为导航目标并安全揭示、滚动一次，但不激活 focus；Runtime 必须回执当前 region 确有可见几何后才提交该导航目标，无法定位时顺序尝试下一处。首次定位未完成前，用户的滚轮、触摸、指针或键盘输入立即取消命令并接管；后续 frame ready、重绘或状态投影不得重复定位。短目标居中，接近一屏或更长的目标展示开头。没有可靠源码关系时，不把“首处变化关联评论 1.8 秒提示”纳入本轮合同，避免凭相近位置臆测关联；评论仍由常驻标记明确进入。用户显式选择目录位置或页边提示时才原子激活 group 与两侧 region。单双页共用同一个缩放轴。页面预览三个按钮使用紧凑的固有宽度。单双页都让文档视口尽量铺满 Canvas：除 pane 边框和双页分隔外只留极小间隙，不保留装饰性大留白。

审阅工具不再占用 Canvas 内部空间，也没有展开/收起把手。它们与编辑/预览/审阅模式、项目、文件动作和 AI 入口同处第二层工具栏；进入审阅时在常驻工具栏槽位挂载“变化 N 处”目录与页面、滚动、缩放控件，退出后移除；主工具栏高度和模式选择位置保持稳定，不因占位控件增减移动画布或改变聚焦测量。窄窗口中会话面板覆盖审阅画布右侧，不继承宽屏的 Grid 列定位，关闭控件必须保持在视口且不被目录或页面内容拦截。上下文可见度不在工具栏逐次调整，而由“设置 → 常规”的两个全局偏好保存。每个文档标签分别保存页面、聚焦、页内 Tab/折叠、双页滚动、画布横向位置与缩放；切回时必须先核对同一 Review session/document，再按页面状态 → 几何稳定 → 滚动/缩放顺序恢复，不能重播首次自动定位。不同标签的 session、group、region、缩放和迟到回调不得串用；Candidate 已采用、放弃、失效或身份变化时删除对应恢复对象，不能恢复旧 Review。

审阅评论标记优先放在目标元素可用空间更大的一侧，打开的评论气泡优先朝远离目标的方向展开；没有一侧能完整容纳时，选择遮挡更少的一侧。标记与气泡跟随页内及画布横纵滚动，尽量贴近目标；只有在修改内容铺满可用画布时才允许局部遮挡。
...
变化事实由分析器按 `text` 与 `structure` 记录，审阅始终展示完整事实集合，不再提供类型筛选能力。页面上的变化标记是区域内明细事实和定位入口；全部事实为空时只在画布原位显示空态。两侧页头共用白底，修改前保持中性标识，AI 修改后使用紫色顶边与紫点，让横向滚动后仍能立刻分辨哪一侧是新版本。
...
修改前 pane 的右边缘是 React 拥有的固定评论轨道。标记横坐标不随 HTML 横向滚动，纵坐标跟随目标；只有显式全局评论停在轨道顶部，运行时 `body` 安全锚点评论仍跟随实际表格或图表。同一源码目标的多条评论共用 `评N`；不同 Stable ID 即使缩放后靠近也保留独立标记，必要时错开标记保持可点击，不能合并评论正文或目标框。Hover 与键盘 Focus 展开原评论 Bubble，并通过私有 frame 端口同时高亮修改前目标和修改后同 Stable ID 元素；删除目标只高亮左页。父级维护 active key Set；marker 卸载、文档替换、frame reload 与 port 关闭都发送 inactive，评论正文和附件始终不进入 authored HTML。

变化目录与页边提示使用克制的紫色位置线索。顶栏中的目录入口必须位于 Electron 可点击区，不能继承窗口拖拽命中。选择入口会更新 `navigationTarget`，并原子写入一个 `activeFocusGroupId` 与两侧各自的 `activeFocusRegionId`；随后按 region 自带的 `presentation` 揭示 Tab/Details，再分别滚动到对应 region。缺失的一侧不改 presentation、不滚动、不生成空遮罩。相同 simple CSS 规则可共享一个 group，并按阅读局部与具体 owner 形成多个可选择 region；多个 region 可以共用一个页边密度提示，但目录必须保留每个带短内容线索的精确位置。每个 inline-style attribute 始终是单目标操作，不能因 property delta 相同而合并。任何样式规则都不得把多个目标提升成共同父容器框；`html/body/main` 也不得成为视觉边框目标。
...
提交回执后的内容读取暂时失败时，保留同一次采用决定并自动核对。只有新内容、版本 Hash 和完整工作文件身份均通过校验，才一起发布新当前稿并清除旧稿的保存错误或冲突；实际外部内容不一致仍保留冲突，不解除编辑保护。
...
点击任何 Vn，包括最新版本，都在该项目的独立历史标签中设置精确历史身份，读取对应不可变快照并核对Hash。历史页直接使用工作台的“预览”模式，“编辑”禁用并说明继续编辑入口；不额外显示“只读”模式。历史不替换当前稿、评论或当前文档会话。

历史页的挂载只依赖当前可见历史标签与精确匹配的历史快照，不要求任何当前稿 Runtime 存在，也不会为了显示历史而补建当前稿。

同一历史标签从 V3 切换到 V5 时，在 V5 的字节、Hash 与画布还没有完成核对前，标签、左侧选中和画布都继续表达 V3。只有验证成功才原子提交为 V5；读取、身份或画布失败时仍保留 V3，不先改标签再回滚。

历史页不显示上方滑出的“创建新版本并编辑 / 回到当前稿”横条。回到当前稿通过已打开标签或左侧栏入口完成；缺失快照或Hash错误不能回退显示当前HTML、另一Version或旧缓存。动态传输失败可提供受限静态显示与刷新。

历史“导出此版本…”复制所见快照。更多菜单保留但禁用“在 Finder 中显示”；“在浏览器中打开”可用，且只打开所见不可变 Version 的经验证文件，不读取、写入或冒充当前稿。隐藏快照仍不作为可编辑用户文件发布。

### 11.3 回到当前稿

“回到当前稿”或独立当前稿行打开项目真实当前HTML，先核对当前身份、源状态和Hash，再按既有会话/画布边界接续。即使当前稿和最新历史完全相同，也不能把二者合并为同一个导航目标；不可用时保留真实恢复状态，不用历史快照冒充当前稿。

旧版本已写入的固定执行进展文案只用于显示；同一回执的身份和事件时间通过核对后，文案措辞变化不阻止打开已验证的当前 HTML。Request、Candidate、历史摘要和当前 HTML 的身份、内容及 Hash 仍按各自权威边界核对。已登记项目确实打不开时，顶部直接说明“当前稿未打开”，保留原页面；不提供会打开新项目选择器的“重新选择”。

### 11.4 从历史创建新版本

历史页的“更多 → 基于此版本创建新版本…”先确认“基于Vn创建新版本”。取消保持历史与当前稿不变。确认时验证项目空闲、所选Version/快照Hash、当前稿Hash及稳定操作身份；先保护被替换前稿件，再建立下一Version并更新同一当前稿。
...
打开已创建版本时，若新 Working Copy 的项目、Document 和 Version 权威已同步发布，导航必须先原子提交当前稿页面，再开始画布确认。若最后画布确认失败，必须由当前稿标签承接错误与重试出口；不得在已显示新当前源时仍保留历史标签或恢复旧历史身份。当前稿标签先前被关闭时，同一打开流程重建该项目的唯一当前稿标签。

从历史标签或左侧当前稿入口恢复同一已创建版本时，必须携带完整、已验证的 Working Copy/OpenTarget 身份，并复用正在执行的导航事务提交当前稿标签；不得在该事务内部再次进入公共导航队列。创建命令已经在写入回执前保护被替换的当前源，重试打开不得再次 drain 已被该创建操作替换的旧源。完成后后续标签导航必须继续可用，且不能重复创建版本。
...
历史预览不提供“继续编辑”或激活旧工作副本入口。用户从历史创建新版本时，当前流程保留可恢复的当前稿，并以一个新的不可变 Version 替换同一 editable Working Copy；保存中断、事务中断、超时和重复提交仍按当前 operation identity 恢复。旧项目副本、旧激活回执和旧格式事务不会被读取、转换或回放。

常规菜单不再提供“找回此前的稿件”，也不挂载保留稿选择对话框。现有保留记录、事务恢复、保护被替换稿的存储与恢复校验继续保留；删除入口不清理用户数据。除非有新的明确产品决定，不因旧测试或历史描述恢复此入口。参见 ADR 0078。
...
| “更多”菜单普通刷新成功 | 不显示顶部成功提示；画布与当前模式的更新即为结果，失败仍进入全局反馈 |
...
| history 模式 | 项目历史标签与预览模式显示精确 Vn；编辑禁用，不增加顶部横条 |
...
执行过程按时间顺序留在对话流中。用户消息使用人物头像及“我”；Stemmio 的机械性交付、接收与准备审阅阶段事实仍保存在 Conversation，但不再逐条作为可见消息插入；结果、失败和决定仍使用品牌头像及名称。过程默认折叠为一行最新非空公开段落预览；预览只整理空白，
由视觉省略号收尾且不跑马，无公开正文时保留现有三点并不生成空折叠控件。Agent 面向用户的公开说明逐条
积累，所有消息严格按 Conversation sequence 显示，最新事实位于最下面；不得按说话人重新分组。用户手动
展开或收起的本轮选择在流式更新、状态变化和标签回访时保留，历史过程同样轻量折叠并保留时间顺序和事实边界。
公开过程封存使用 typed `process-summary`，以 Request / Attempt 与实时过程共用同一阅读节点和折叠选择，
不额外生成结果消息；封存后按 Conversation sequence 排列。typed `result-summary` 与 Stemmio 校验、失败、采用事实始终正常可见；普通 Agent 文本按消息类型进入过程，
不通过关键词猜结果。Agent 的服务名、计时与已接收 KB 归属于同一条当前 Agent 消息的头部，不另建状态段
或一条常驻的 Stemmio 状态压在其下。Agent 结束后，后续校验与 Candidate 事实再按发生顺序追加。展开区域
与消息流共用纵向滚动；复制取完整脱敏正文，不复制预览或元数据，只有真实截断才提示省略。上滚、选择文本、
展开历史或折叠高度变化均保留用户操作的触发器锚点，不抢滚动；用户明确上滚离底部至少 120px 后显示“回到最新”，轻微上滚或单纯展开过程不显示。离开底部阅读时收到新事实则显示“有新进展”，点击后恢复底部跟随。侧栏重挂载按打开标签
恢复临时阅读位置，明确新 Request 才回到底部，关闭标签即清理。停止操作仍固定在底部。侧栏与画布只保留
1px 视觉分隔线，18px 透明拖拽命中区不另画粗条。
...
- 导出对话框每次默认勾选保存历史版本，用户可以取消勾选；勾选后以本次导出相同字节建版，取消和导出失败不建版。
- 导出成功而建版失败保留成功文件事实，恢复入口只重试同一建版操作。
- 常规菜单无找回稿件入口；底层保留记录与失败恢复保护不变。
...
暂时不自动发送或排队，界面不重复提示草稿保存、修改意见条数。发送主按钮统一称“交给 AI 修改”。标题栏右侧以复制图标提供“复制任务指令”，其载荷仍是指向本地 Prompt 的任务交接指令；原关闭按钮由工作台“AI 助手”按钮的再次点击替代。历史日期分组不附服务或模型名称。待审阅/失败等需要用户
决定时，在输入组件上沿展示附属交互层；候选就绪卡只保留状态标题与“查看修改”等实际动作，尺寸紧凑，并在首次出现时轻微上浮后落定，减少动态效果偏好下取消动画。执行中停止在输入组件内，决定结束后
交互层收起。键盘焦点和输入不被交互层出现打断。

会话可见过程保留 Agent 的公开说明与有意义的 Stemmio 事实，按 Conversation sequence 排列；机械性的准备、交付、接收和校验进度只保留在底层事实，不重复显示。历史过程默认折叠，
展开状态属于打开标签的临时阅读状态。公开说明以消息/段落边界保存，实时、完成后和重开使用一致的断句展示，
当前过程默认显示最后一个非空公开段落的单行预览，复制保留完整脱敏正文与原文换行。时间/复制只在对应消息
hover 或 focus-within 时出现，不挤动正文；跟随、锚点与“有新进展”恢复只影响侧栏展示，不改写 Conversation、
Request、Attempt、Document 或 iframe 生命周期。
