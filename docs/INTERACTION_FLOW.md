...
- 一个 `projectId + documentId` 在标签栏中最多出现一次。切换文档标签必须经过现有
  ...
  都不常驻完整静态 iframe。当前默认停用切页时的静态 HTML 展示交接：切换直接进入正常
  Registry/OpenTarget 打开链路，由唯一正式 Canvas 展示新页面；HTML 数据缓存仍用于有界投影和阅读状态，不预先显示第二张只读页面。正式 Canvas 准备前允许短暂等待，以避免静态页与正式页先后排版造成可见跳动。可回退的交接实现保留，仅在显式 E2E 诊断开关下启用：命中缓存标签时临时脚本禁用面位于画布列并继承正式宽度，`onLoad` 报告含 `tabId + sourceSha256` 与本次导航身份的精确 token；旧 iframe 的迟到回调不能完成新交接。`data-display-ready` 等属性仅供测试和诊断，不能作为生产控制链。精确 Canvas 接管后临时面释放。
