...
- 一个 `projectId + documentId` 在标签栏中最多出现一次。切换文档标签必须经过现有
  ...
  Registry/OpenTarget 打开链路；不命中时不为制造展示缓存而推迟正式打开。静态面的 `onLoad` 直接报告精确候选 token 给唯一交接 owner；token 同时含缓存的 `tabId + sourceSha256` 和仅本次交接有效的导航身份，因而同一 Hash 的下一轮导航不会接收旧 iframe 的迟到回调。`data-display-ready` 等属性仅供测试和诊断，不能被父组件观察为控制链。精确 Canvas 接管后交接面立即释放，快速反向切换最多短暂重叠旧面与新候选面。
