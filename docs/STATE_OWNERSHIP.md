...
| Read-only tab source projections and exact-version per-tab Canvas mode/PageViewContext/scroll restoration | Controller-owned `DocumentSurfaceCacheSession` owns byte-bounded HTML data plus separately retained light presentation state. It never mounts a DOM or iframe. Workbench's pure `decideDisplayHandoff()` and `useDisplayHandoff()` combine the current intent, exact retained surface identity and existing Edit/Preview proof into the sole presentation decision; they never own source, Runtime execution or navigation authority. The decision's actual surface also owns the displayed tab and mode projection until the destination is revealed. `WorkbenchActiveDocumentCanvas` and `WorkbenchActivePreview` retain their exact outgoing DOM inert while the destination prepares; a Preview changing between active and outgoing roles keeps its physical iframe. Workbench separately retains one exact same-document Preview iframe for at most 3 seconds after Edit settles, fenced by source identity, attempt and physical mount; this is transient process memory, not a cross-tab cache. `WorkbenchNavigationWorkflow` only touches/removes projection entries | none; bounded process memory only, 20 HTML entries and 32 MiB of source projections, per-open-tab bounded presentation state. Normal handoff has at most two transient editors, one inert outgoing and one hidden incoming; settled Edit has one writable Canvas and may briefly have one hidden inert Preview whose scripts still run. Inactive tabs retain no iframe, editor or Runtime DOM | Normal switches reveal the incoming Canvas only after its current generation and exact source SHA verify and its same-path, same-generation runtime has settled, declared no script candidate or explicitly fallen back; Preview supplies its own exact-instance display/degradation result. Failure removes the outgoing surface and presents recovery. Cross-document outgoing clones have no edit, save, export, comment or runtime callbacks; same-document retained surfaces are inert until their own target is revealed. |
...
| Current source bytes, disk-confirmed Hash, working-HTML Hash, edit revision, persistence projection, pending and active write, single-flight source flush, Canvas-rendered Hash, monotonic context-bound source receipts with session incarnation, exact-byte boundary reconciliation and protection evidence | Renderer `DocumentSession` owns current bytes/state and receipt sequence/incarnation. Its production write surface is limited to complete edit/write/failure/authority/reset actions; it exposes no arbitrary patch or raw field setter. Accepted HTML, revision, receipt and ordinary pending write publish atomically. After `DocumentWorkflow` validates an external response, one Session confirmation accepts the exact active write, decides current-document completion and atomically publishes the proven persisted/working Hash and any verified route authority. Same-byte identity rebases remain explicit. `DocumentWorkflow` owns external I/O/response validation, reload/repair/external-accept operation identity and single flight, exact confirmation/preview evidence, cross-owner routing/recovery coordination, revision/context-bound verified recovery/export receipts and operation-local leave-boundary decisions | source HTML and runtime autosave record; Main owns the atomic per-document recovery journal; journal path is a CAS-rebased location, and the Canvas generation itself is disposable | Canvas returns immutable observations for presentation/cache reuse only. `DocumentWorkflow` alone confirms them. Source replacement reports permission, source acceptance and page restoration independently; after acceptance a failed Canvas stays repair-required without reverting source. Projection repair joins Document's save flight and never reads disk. Only the exact synchronous freeze checkpoint's validated save ACK may advance its operation context; HTML, revision, receipt, route and session incarnation remain fenced. The Canvas owns a per-instance freeze ID; Workflow cleanup may release only its own acquired freeze and respects current controlled access. Old operation completion/finally cannot mutate or unlock a newer context. `beginWrite`, restore, confirmation, failure and flush completion are identity-fenced: an old write may advance only its proven persisted revision/Hash and cannot clear a newer active/pending edit, failure, session or flush. Source persistence may be idle while the matching flush still retires its recovery journal. Every authority receipt, including a same-byte registration or locator transition, advances Canvas generation and requires a fresh physical frame. Save/export/AI/leave consume complete Working HTML and exact persistence or recovery evidence; a stale rendered projection remains a distinct honest fact; an in-flight operation may reuse an already verified exact receipt/HTML/Hash/generation observation as completion, while duplicate public confirmation still returns false and mismatched observations never certify recovery |
| One user-authorized default-browser open flight for the selected current draft or immutable historical Version | Renderer `BrowserOpenWorkflow`, composed by `WorkspaceController`; Project/Document/Version Sessions remain the fact owners | none; current opens use the exact flushed revision and settled persisted Hash, while history opens use the selected verified Version Hash. Desktop independently reauthorizes the known path and rereads that Hash before `shell.openExternal` | Workbench dispatches one command and presents the result. Duplicate same-target clicks share the flight; navigation, later edits, failed/unknown persistence, malformed identity and Desktop refusal do not launch. Finder and export retain their existing owners |
| Force-unlock of a Working Copy conflict (adopt the exact previewed disk Hash as `saved`, no user-HTML rewrite beyond Stable ID materialization; clear `runtime.activeRequest` if present; keep `lastPersistedRevision`) | `ProjectFileRepository.forceUnlockWorkingCopy` via `POST /conflict/resolve` `force-unlock`; before mutation it requires exact `operationId + projectId + documentId + sourcePath + expectedSourceSha256` and writes one bounded pending receipt into the Working Copy state. Adoption remains `saving` until identity CAS succeeds, so ordinary workspace reconciliation cannot accept a different disk Hash in that window. `force-unlock-result` queries only that operation and distinguishes the previewed accepted Hash from the final identity-materialized Hash; it never repeats the mutation to mask an unknown reply. A different operation must first settle the single pending slot; `superseded` is durable and repeat-queryable before the slot may be replaced | Working Copy state record, its single validated `forceUnlockReceipt`, and runtime request pointer | `DocumentWorkflow.acceptExternalConflict` / `adoptShownExternalPreview` and the existing conflict banner; Workbench supplies only typed intent/receipt and retains an unknown operation until its durable result is known. Run keep-external permits the original pending operation to reconcile before treating the final materialized preview Hash as a new conflict |
...
| Managed Working Copy source-element identity schema, sealed binding Hash, one-time materialization and crash recovery | `ProjectFileRepository` (`working-copy.mjs` supplies pure inspection/materialization/binding and CAS paths; the façade owns transaction sequencing) | current Working Copy HTML, `working-copy-state.v4.sourceElementIdentitySchemaVersion + sourceElementIdentityBindingSha256`, manifest file identity, the latest auditable committed migration transaction for that Working Copy, and temporary complete before/after recovery bytes; settlement/restart cleanup retires older committed migration records | editable workspace hydration; external clean edits must preserve the sealed ID/tag/parent/order binding or require explicit force-unlock; `IslandEditingController` and the text-range style planner preserve existing IDs and allocate them for new inline source descendants, while normal save verifies every prior claim and fills only valid new-element omissions; historical Versions, external originals, Runtime DOM, comments and Review are not consumers with write authority |
...
| 源页 Agent session Token and remembered-credential operation | Bridge `AgentRuntimeCoordinator` owns the live Token; Renderer `RunWorkflow` owns one provider-scoped credential intent plus only the short-lived connect/retry input and latest `operationId`; Main credential store owns durable accept order, record ID, receipts and tombstone | process memory for the live session; optional Main `safeStorage` ciphertext under `userData/agent-session-credential.v1.json` only after explicit remember, including non-secret Custom `modelId`; never plaintext secrets on disk, `ui-preferences.json`, logs, GET responses or Catalog snapshots. Replacement, disconnect, remove and disposal retire obsolete held references; terminal saved/missing/superseded/rejected receipts retire their operation reference, while unknown/unreadable/unavailable may retain only the reference needed to reconcile or retry that operation | Workbench forwards one command through the typed credential port and never holds/replays the Key. The pure credential interpreter requires explicit `saved`, the exact queried operation ID and a legal record ID; `remembered` alone has no authority. Startup status and connect/configuration/persist/default effects share a generation fence. An unknown clear remains one pending operation and a later remove queries that operation before any new clear |
...
| Development synthetic provider/runtime fixture and Qoder ACP live-probe report | `tests/fixtures/agent-provider/qoder-provider.mjs` provides path-free registry evidence; `scripts/qoder-acp-spike.mjs` uses the shared ACP runtime only against its disposable synthetic Project File | no product authority; the fixture is process-only and ignored `output/qoder-acp-spike/report.json` is diagnostic evidence only | provider contract tests and developers evaluating live account/network compatibility; never a release gate or Candidate authority |
...
| Volatile interactive-preview document, bootstrap, allowed source-relative asset root, completed-frame identity set and one-way pre-load scriptless navigation-fallback flag | Main-process preview protocol controller plus the owning window's navigation fence | none; bounded in-memory session/window state only; the fallback cannot be reversed inside a session. Read-only session inspection proves that a retained Preview resource session has neither expired nor been evicted, without extending its lifetime | isolated preview iframe and the script-disabled edit iframe's resource base |
| Current preview/edit display context, safe reveal transition and per-surface render acknowledgement | Workbench page-view context state | none; source-bound in-memory projection tagged by `DocumentSession` Canvas generation, source receipt and rendered source Hash. Edit reveal separately requires the current physical frame's settled Runtime grant or script-free static proof; a safely projected edit may keep an older Runtime startup Hash without claiming that program ran again | `HtmlCanvasEditor`, `HtmlInteractionPreview` and toolbar |
...
| AI review page view, context visibility, navigation target, canonical page-presentation path, scroll mode and zoom mode | `AiReviewWorkspace` review reducer | none; disposable state bound to the frozen before/after pair | review toolbar, complete change directory and isolated review frames |
...
- `AgentCatalogState` owns one public configuration and status projection per provider in its existing map;
  ...
  and requested reasoning through `RunWorkflow`'s narrow adapter to the same
  `WorkspacePreferencesSession` used by Settings. The
...
- AI review state fields are orthogonal. Page, visibility, navigation,
  ...
  second fact-set or mask owner. The parent owns the full nested panel path and
...
Workbench 的跨项目规则/历史意图由统一 Navigation admission 持有：ProjectWorkflow 只读取 Registry 已验证的 OpenTarget 并生成一次不可变 `ProjectSurfaceContext`，不发布目标 `ProjectSession`、`DocumentSession` 或当前稿标签。规则读取先保存在 `ProjectRulesWorkflow` 的一次性准备结果中；Navigation 成功提交目标规则标签后，才同步发布同一上下文的规则会话，任一步失败都保留原标签、原规则内容及原保存目标。Version owner 以该上下文读取精确历史页；历史预览的挂载不依赖当前稿 Runtime。失败保留原页面，不能先显示目标当前稿、借另一 Version 顶替或由 React effect 重放第二条导航。规则编辑、保存、重试、历史浏览器打开与建版都校验各自可见页面上下文；另一项目的 AI 锁不能扩大为全局锁。
...
AgentRuntimeCoordinator emits bounded, fixed-category execution facts through its injected repository writer. The start fact must persist before invoking a provider. Stage facts are serialized independently of Renderer mounts; a persistence failure aborts execution and disallows automatic retry. Raw provider text, arguments and output do not enter this history path. On execution settlement, the already-redacted ordered public blocks are joined with `\n\n` only at the summary boundary, then sealed and bounded to 4096 characters; hidden reasoning and tool output remain excluded. The existing submission receipt replays this immutable summary after restart; Request and Promotion outboxes continue to own result and decision facts.
...
same message. Its only public narration representation is the ordered, stable-ID
`visibleTextUpdates` list (at most 80 display updates); no cumulative `visibleText`
field is forwarded through runtime, Bridge or Renderer. Earlier updates collapse
without losing text. A full string is derived only at an explicit copy or summary
boundary by joining those already-redacted blocks with `\n\n`. The execution narration
limit is 65536 UTF-16 code units, including display separators and redaction
expansion; runtime byte limits remain separate. The 4096-character summary is
derived at that boundary with explicit truncation. Run progress stages come only
from the domain presentation. Hidden reasoning and raw tool arguments never enter
the public event allowlist. Tool activity is translated from known event categories
...
Explicit source reload additionally checks the verified physical frame generation: identical source bytes in the previous frame cannot acknowledge recovery. Workbench waits for an admitted author candidate; a settled static/failure outcome uses the existing bounded static rebuild. Frame promotion transfers keyboard focus only when the retiring Canvas owned it and a fresh post-positioning read still finds the old Active iframe, the promoted iframe or neutral outer-document focus; a newer comment/composer or other input focus wins. The sole caret-reconstruction exception is the source-accepted, receipt-bound `NativeEditRecoveryController` intent, and it uses that same fresh focus gate before one-shot consumption. Commit rollback returns focus only if the failed candidate still owns it.
