...
Workbench 的跨项目规则/历史意图由统一 Navigation admission 持有：ProjectWorkflow 只读取 Registry 已验证的 OpenTarget 并生成一次不可变 `ProjectSurfaceContext`，不发布目标 `ProjectSession`、`DocumentSession` 或当前稿标签；规则/Version owner 以该上下文读取精确页面，成功后才提交对应项目作用域标签。失败保留原页面，不能先显示目标当前稿、借另一 Version 顶替或由 React effect 重放第二条导航。规则保存、重试、历史浏览器打开与建版使用各自页面上下文；另一项目的 AI 锁不能扩大为全局锁。
