...
| 源页 Agent session Token and remembered-credential operation | Bridge `AgentRuntimeCoordinator` owns the live Token; Renderer `RunWorkflow` owns only the short-lived connect/retry input plus latest `operationId`; Main credential store owns durable accept order, record ID, receipts and tombstone | process memory for the live session; optional Main `safeStorage` ciphertext under `userData/agent-session-credential.v1.json` only after explicit remember, including non-secret Custom `modelId`; never plaintext secrets on disk, `ui-preferences.json`, logs, GET responses or Catalog snapshots | Workbench forwards one command through the typed credential port and never holds/replays the Key. `RunWorkflow` independently reports connection, persistence and default commit, reconciles unknown save results by operation status before retry, and owns disconnect/remove/reconnect sequencing |
...
| AI Request freeze, Agent access orchestration, pre-Request use-time check, persisted-boundary verification, safely fenced same-Request Agent start/retry, unknown-POST authority reconciliation, polling lifecycle, cancellation ordering, conflict command sequence, pending sidebar default adoption and access-repair resend | Renderer `RunWorkflow`, composed by `WorkspaceController` | connection, remembered-credential persistence and default preference are separate results. The validated pending default is written through the narrow preferences port for its exact `intentId`; the single preferences Session rechecks that intent before and after its serialized write and restores the prior durable default when an in-flight result is superseded. After Request publication every path reads the durable Request selection. Access-repair identity is `{repairIntentId, requestId, attemptId, projectId, documentId, sessionEpoch}` in process memory | Workbench sends intents only; typed credential/preferences ports, Catalog projection and Bridge run lifecycle |
