...
| Read-only tab source projections and exact-version per-tab Canvas mode/PageViewContext/scroll restoration | Controller-owned `DocumentSurfaceCacheSession` owns byte-bounded HTML data plus separately retained light presentation state; `useDocumentSurfaceHandoff()` is the single accepted-presentation owner, including the full token of the physical iframe it accepted, while Workbench mounts a cached `HtmlDisplaySurface` only for an explicit tab handoff and owns exactly one active `HtmlCanvasEditor`; `WorkbenchNavigationWorkflow` only touches/removes projection entries | none; bounded process memory only, 20 HTML entries and 32 MiB of source projections, per-open-tab bounded presentation state, at most two transient static display iframes during rapid handoff overlap, one active Edit Canvas and its bounded editor-internal A/B handoff slot; settled inactive tabs retain no iframe, editor or Runtime DOM | a pending tab may show an exact script-disabled cached frame while canonical registered-project open validates the sole editable authority. Its iframe mounts only after the final resource base is known and keeps one immutable `srcDoc`; `HtmlDisplaySurface` directly reports a token containing the cache identity plus the current navigation handoff identity, and the owner accepts or rejects it. The accepted physical iframe remains visible while a different candidate loads; the candidate is hidden until its own full token is accepted, even for a same-Hash repeat navigation. That ephemeral identity fences same-Hash later navigation rounds without entering cache state. Its `data-*` readiness attributes are diagnostics/test evidence only, never a production control channel. The cover releases only after a current-generation Canvas authority; when navigation published a SourceReceipt, it must match that exact receipt, and otherwise the `DocumentSession` authority remains its own SourceReceipt fence. A same-Hash observation from another generation never retires the cover |
...
Replay keeps the previously stored wording of fixed Stemmio execution captions when their turn, Request, attempt, candidate and event time still match the receipt. This wording is presentation and may change between builds. Submission requirements, public Agent summaries, event identities and their authority remain exact; a reused identity with changed ownership or content is still rejected. A caption-only compatibility difference cannot prevent opening an otherwise verified current Working Copy.
