...
| Force-unlock of a Working Copy conflict (adopt the exact previewed disk Hash as `saved`, no HTML write; clear `runtime.activeRequest` if present; keep `lastPersistedRevision`) | `ProjectFileRepository.forceUnlockWorkingCopy` via `POST /conflict/resolve` `force-unlock`; before mutation it requires and matches exact `projectId + documentId + sourcePath`, then rechecks `expectedSourceSha256`, and returns the accepted bytes/Hash/state | Working Copy state record and runtime request pointer | `DocumentWorkflow.acceptExternalConflict` / `adoptShownExternalPreview` and the existing conflict banner; Workbench supplies only typed intent/receipt |
