...
`submitting`、`processing`、`validating`、`committing`、冲突和事务恢复均锁定当前项目。`ready-to-open` 表示新版已经安全生成但尚未成为当前源；界面默认突出“审阅对比”，同时保留“直接打开”。正式审阅页在无保存与激活权限的隔离沙箱中展示冻结当前版与 AI 候选。进入审阅默认显示“双页 + 同步滚动 + 适应画布”总览，并始终展示全部变化事实；第一次只把第一处变化作为导航目标并定位，不激活遮罩或边框。目录导航、局部聚焦、页内运行态、滚动与缩放彼此独立；变化聚焦与评论聚焦的上下文偏好在设置中保存，默认分别为 25% / 15%。
