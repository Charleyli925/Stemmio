...
| Version and history | `VersionSession` owns immutable records and verified history preview bytes; `DocumentSession` remains the current working source | `VersionWorkflow` owns local version save, recovery, exact HTML export and operation reconciliation; history is a verified read-only projection that mounts without a current-draft Runtime | `version-workflow.js`, `version/review-plan.js` |
...
| Agent connection, remembered credential and preference commands | Bridge Coordinator owns the live session Token; Main `desktop/agent-session-credential-store.mjs` owns optional `safeStorage` ciphertext and ordered receipts; the single `WorkspacePreferencesSession` owns renderer preference writes | `RunWorkflow` owns one provider-scoped credential intent across connect/configuration/persist/default and provider-disabled writes, fences startup status, and reconciles an unknown clear with the same operation ID. `agent-credential-operation.js` is the pure receipt interpreter; `AgentCatalogState` projects public status plus its operation kind only. Workbench supplies typed ports and never retains the Key or persistence callbacks | `run-workflow.js`, `agent-credential-operation.js`, `agent-provider-catalog.js`, `workspace-preferences-session.js`, `desktop/agent-session-credential-store.mjs`, `shared/agent-vendor-key-url.mjs` |
