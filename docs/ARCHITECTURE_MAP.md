...
| Agent connection, remembered credential and preference commands | Bridge Coordinator owns the live session Token; Main `desktop/agent-session-credential-store.mjs` owns optional `safeStorage` ciphertext and ordered receipts; the single `WorkspacePreferencesSession` owns renderer preference writes | `RunWorkflow` coordinates connect/persist/default, retry/status reconciliation, clear/restore and disabled-provider commands. `AgentCatalogState` projects public status only; Workbench supplies typed ports and never retains the Key or persistence callbacks | `run-workflow.js`, `agent-provider-catalog.js`, `workspace-preferences-session.js`, `desktop/agent-session-credential-store.mjs`, `shared/agent-vendor-key-url.mjs` |
