...
- Finding SD-001 / ACCESSIBILITY-BARRIER / P2 / medium (source): 动态内容提示固定在窗口顶部且含关闭与恢复按钮，原先没有退出 macOS 拖动命中区；补 `no-drag`，并将现有 Electron 用例的关闭操作改为按钮可见坐标的鼠标点击。
- Finding SD-002 / ACCESSIBILITY-BARRIER / P2 / medium (source timing): About、结束 AI 本轮和图片预览的原生对话框在 `showModal()` 后下一帧才聚焦，存在焦点落在鼠标按下与抬起之间的风险；改为打开时立即聚焦安全默认按钮。历史建版确认已有相同处理。
- Finding SD-003 / ACCESSIBILITY-BARRIER / P2 / medium (source): 审阅确认遮罩覆盖标题栏但没有 `no-drag`；补遮罩命中规则。找回保留稿对话框原先不能点遮罩关闭；现在与同类对话框一样可通过遮罩、`Escape` 或“关闭”退出。原生对话框及其遮罩统一退出标题栏拖动区。
- Copy: 全局通知仍由封闭的 `globalInterruptionPresentation()` 决定标题、时长和动作；“选择附件”与“选择导出位置”明确了各自打开的系统选择器。危险操作确认、持续文件安全状态与原位设置确认保留各自的操作语义，不新增全局提醒。
- Evidence: 已重新构建桌面 Renderer，并在真实 Electron 中用鼠标点动态内容提示的关闭按钮，以及找回稿件和 About 对话框的标题栏遮罩；三条路径均关闭成功。`npm run task:finish` 在本次代码上通过（Browser 83/83、Electron 124/124、AI 16/16）；固定系统文案变化与错误请求身份的回执正反例也通过。其余入口按当前源码、既有交互合同和选中用例盘点，未逐一做鼠标故障注入；未对用户项目或真实 HTML 做故障注入。

## 2026-09-23 — 当前稿打开兼容与顶部提醒

- Truth: 固定执行文案的旧措辞按原历史显示；回执归属和用户／Agent 内容仍核对。已登记项目的当前稿打不开时直接说明结果，不再让“重新选择”打开新项目选择器。需要选文件的入口写“选择 HTML”，同一外部请求的继续动作写“继续打开”。
- Electron: 在隔离合成项目中暂时移走工作文件，通过“在 Finder 中显示工作文件”触发真实顶部提醒。实测提醒的 `app-region` 为 `no-drag`，鼠标点击关闭按钮的可见坐标后提醒消失；合成工作文件随后恢复并清理。未触碰用户项目。
- Boundary: 本次验证的是源码构建的 Electron 和合成项目；已安装 Developer Preview 仍是旧构建，未执行打包或更新安装。

## 2026-09-24 — 当前稿切换与预览

- Truth: 点击另一当前稿后，旧画布原位留到新正式画布的当前代次和源码 Hash 核对成功；留存画布不可操作且没有编辑、保存、评论或 Runtime 回调。失败时撤下旧画面并提供重试。预览仅按当前选中文档身份、权威内容及正式审阅状态判断，不等待评论、版本或编辑画布的后台收尾。
- Electron evidence: 定向测试将目标 Canvas 的 `onLoad` 挂起，比较切换前与挂起中的画布上方 600px，像素差异低于 1%，并断言旧画布 `inert`、新画布隐藏、接管后只有一个编辑器。另验证挂起时预览可用且显示目标文档，以及目标读取持续失败后错误与重试可见、重试后恢复。
- First failure: 预览测试最初替换了被 HTML 标签拆开的标题文本，替换没有发生；改为完整文本节点后严格内容断言通过。失败测试最初只拦截第一次读取，被产品自动恢复链继续打开；改为持续故障直到用户点击重试，以核对真正的终态。
- Boundary: 仅合成项目的交互过渡证据；本机真实 HTML 与用户手动点击仍单独验收。未制作安装包或提交 PR。
...
- Refusal copy is now grouped into three user-facing meanings: outside direct-edit scope, cannot move across groups/positions, or the current page content changed and the action must wait. Internal reason codes remain diagnostic and are not exposed as implementation vocabulary.
- The existing More → “从磁盘重新载入 HTML” route now has a single explicit Electron canary that reloads same-byte source, continues Native Edit, saves, closes normally, restarts the same isolated project and verifies the edited text plus Stable ID. This is interaction/continuity evidence only; no pixel artifact is claimed.
- The read-only private-corpus preflight records one privacy-safe first failure per file (stage, code, classification and bounded scalar preconditions) while retaining a capped later-failure list. The eight-file run remained `DISCOVERY_ERROR` for all 8/8: failures were capability-probe/runtime-generated executor conditions (`CAPABILITY_PROBE_HOST_POINTER_INTERCEPTED`, `CAPABILITY_PROBE_SELECTION_NOT_CLEARED`, `NO_EXACT_HIT_POINT`, or `RUNTIME_GENERATED_PROBE_FAILED`); no row was reclassified as unsupported and no target was substituted. Originals and managed Working Copies remained byte-identical.
...
## 2026-09-22 — Clear white workbench across modes and settings

DESIGN CHANGE. Visual truth: the user's selected second white-shell proposal, with the third proposal's unboxed Edit / Preview / Review mode treatment. The scope includes project navigation, all three modes, Settings (General / AI services / Updates), comments and AI conversation. Authored HTML, window geometry, permissions and state owners are unchanged. Native macOS vibrancy is not introduced.

- Implementation: shared neutral tokens live in `tokens-and-base.css`; chrome aliases are rooted in `workbench-chrome.css` so Portal menus inherit them. Fixed surfaces use white or near-white without blur or grain. Project/settings selection is neutral gray. Comments retain their border and anchor connector with a small neutral shadow; the AI composer retains one light shadow. Review keeps the after-side purple line/dot and semantic states.
- Source: base `e3de7d602c8977cc07d98d06ef90b16611128147`; final app + CSS-test diff SHA-256 `d1a6f2353fdceba278980ba856675284ba588cbba48e6e70237f829f9a1c69af` (`git diff -- app tests/workbench-css.test.mjs`). No renderer logic or user HTML changed.
- Real Electron evidence: rebuilt renderer, isolated profile, synthetic business report, 1440 × 960 and 1024 × 768. Captured Edit/comment card, comment composer, Preview, Preview with AI, all three Settings categories, AI result, Review, narrow Review and the portalled More menu. Local captures: `output/design-qa/clear-shell/after/01-edit-comments.png` through `11-review-menu.png`; observations are summarized here so the local files are not the sole review evidence.
- Measured final values: toolbar/comments background `rgb(255,255,255)`, primary text `rgb(13,13,13)`, sidebar `rgb(250,249,248)`, fixed-surface backdrop filter `none`. Each of Edit, Preview and Review exposes its existing selected state and a visible 2px `rgb(90,85,223)` underline. The normal comment card remains white with a 1px neutral border and `0 4px 14px` shadow at 4% black. Narrow Review has zero document horizontal overflow, and the AI close/decision controls remain inside the window.
- Continuity: public UI actions created an anchored comment, switched modes, opened all Settings categories and returned to the document. The managed source bytes were identical before and after those actions. A synthetic copied-task candidate went through the existing finalizer and opened the real before/after Review with the decision panel visible; this was not a real provider request or adoption qualification.
- Iteration evidence: temporary capture setup initially used the wrong fixture marker, then observed old clipboard content before the copy completion, then assumed an optional finalizer field existed. Logs are retained as `output/clear-shell-qa-attempt1.log` through `attempt3.log`; these were capture-script defects, not product failures. The successful initial full capture exposed a real styling defect: Review's tooltip suppressed an `::after` underline. The mode owner now uses a reserved transparent bottom border and selected border color, so tooltips cannot remove the indicator. Final capture assertions and visual inspection passed for all three modes (`output/clear-shell-qa-final.log`, `after/evidence.json`).
- Independent visual review found that the first final Preview capture preceded iframe content readiness (the subsequent AI capture already contained the document). The capture now waits for the report heading inside the Preview iframe before taking the image; the earlier empty capture is preserved as `03-preview-before-ready.png`. This is an evidence-readiness correction, not a demonstrated product regression.
- Existing CSS checks retain layout/cascade ownership assertions; four assertions locking the obsolete blur, gray fill and colored shadow were removed. The changed appearance is verified on the rendered Electron UI rather than by adding CSS-literal mirrors. Task-gate and focused runtime results are recorded separately in PR evidence.
- Documentation: DESIGN_LANGUAGE owns the new palette/material rules; INTERACTION_FLOW describes the unchanged workflows and updated mode/Review identity. CHANGELOG records the visible change. No ADR or product boundary change.

Result: scoped visual acceptance passed. This is source-build Electron evidence, not packaging, installed-app, private-HTML-corpus, dark-mode or release acceptance.

### 2026-09-22 — Full-matrix comment appearance assertions

The first Ready run at `1ba71bb8b8f05d04f417274eeace3399aa870b22` exposed two failures in `native-dom-comment-tabs.spec.mjs`: computed-style expectations still required the retired shadow-free card and purple-gray focused surface. The two tests reproduced locally on that exact source. This is a test-expectation omission in the visual change, not an infrastructure failure; the hosted attempt and local screenshots/traces are retained. Four expected values now match the approved white surface, neutral border and small shadow. Geometry, focus identity, queue order, minimum gap, edit/delete controls and wheel behavior assertions are unchanged. The entire comment-tabs suite then passed 10/10 in 14.3s, including all assertions following the original failures. Product CSS and the prior rendered visual acceptance are unchanged. Final gates are recorded in PR evidence; the failed head is not eligible for merge.

The same Ready run also exposed the retired translucent decision-dock expectation in the provider suite. It reproduced locally; updating its background expectation to white preserved all geometry, resize, keyboard, zoom and adoption assertions, and the full provider suite passed 13/13. A separate hosted accepted-page recovery case timed out after a successful single adoption. Its test-level trace proves the retry click but contains no internal recovery outcome. On unchanged product code, the focused case passed; temporary local outcome diagnostics then observed successful repair, exact Canvas verification and one-shot recovery settlement in 10/10 ordinary runs and 3/3 runs at 6x renderer CPU throttling. These results do not establish an environment cause or fix the unclassified hosted failure. Diagnostic instrumentation was removed. The first trace remains a follow-up investigation limit; the final source must independently pass the unchanged recovery assertion in the complete Ready matrix before merge.

The next Ready head `9130dbb767e528426b8b4d753fa13e5673feda25` passed the corrected appearance checks but exposed an early narrow-sidebar measurement: 242.641px while the 264→240px Grid transition was still running. A temporary 2s transition reproduced the old fixed-220ms wait failure at 253.383px. Waiting for the actual 240px geometry passed the same slowed transition and the unmodified-duration case; the slowdown was removed, and the existing exact-width and subsequent layout assertions remain. The accepted-page recovery timeout also recurred on this hosted head and remains blocking pending diagnosis; it is not treated as an environment failure or closed by earlier local passes.
...
### 2026-09-23 — Global sidebar toggle motion

DESIGN CHANGE. Visual truth: the user's screenshot and reported opening/closing behavior; it is a symptom reference rather than a pixel target for this synthetic project. The shell column previously transitioned over 180ms while the tab strip padding switched immediately, so the Start tab could jump left before moving with the sidebar. Closing also replaced the sidebar button with a tabbar button: a before-fix Electron frame trace measured the button at 86px, then 350px, then back to 86px.

The toggle now stays in one fixed workbench position. The shell and tab strip use the same 120ms transition, and system reduced motion or the app's reduced-motion setting removes both transitions. A rebuilt-source Electron case with a synthetic document and Start tab sampled both directions frame by frame: the Start tab never moved left of its collapsed starting position on opening or right of its expanded starting position on closing, and the button varied by at most 1px. It also verified the tooltip updates across a click and reduced motion is immediate. Static open/collapsed screenshots were visually checked at `output/playwright/native-dom-electron/results/electron-workbench-tabs-si-16490-ng-it-against-the-left-edge/`; an existing Electron shell/tooltip case passed. The source-build screenshots and frame trace do not claim that the installed Developer Preview has been updated or establish identical frame timing under every user HTML workload. Final result: passed for the scoped source-build interaction.
