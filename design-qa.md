...
- Runtime Candidate promotion of the post-commit SVG selection intent is `NOT_EXECUTED`: the Browser persistence double retained an in-place Runtime at generation 3 instead of deterministically starting a second Candidate. The observed failed run proved only that the SVG was structurally selected and the just-entered token reached the authoritative source; it is not counted as promotion coverage, and the unreliable synthetic scenario is not part of the default suite.
