...
- 简化当前稿菜单与导出选项，拉开菜单标题和说明的层级；刷新统一读取保存的当前稿且成功时保持安静，文字编辑支持首次外部单击退出，标签页以浅紫活动线反馈加载、编辑和 AI 处理。
...
- 等待切入的当前稿标签从准备状态起显示稳定紫色底线；旧画面仍在屏幕上时，原标签与模式按钮保持正式选中态，目标就绪后再与画布一起切换，并向辅助技术说明打开状态。已选中标签内切换历史版本不改变标签名称。目标预览的眼睛图标在加载期间原位转圈，页面就绪后恢复，不再浮动显示加载文案。
...
- 欢迎页预览切到 T1 编辑时，交接中的欢迎页保留原预览 iframe 与运行会话，避免旧页重新挂载或旧编辑画布插入可见画面。关闭新建的“开始”标签返回原编辑页时，复用仍在的画布和 Runtime，不再次打开同一当前稿。
