...
- 修复 Developer Preview 中已写入项目的空历史激活占位字段和旧脚本评估结论导致历史不可读，并恢复已发布密文凭证的安全读取；旧脚本评估会依据密封 HTML 在内存中转为“需要审阅”，非空旧回执、无关评估漂移、明文或损坏记录仍拒绝。
...
- 审阅默认适应画布，“变化 N 处”移到顶部审阅入口旁，并移除低频且容易误导的全部/文字/元素筛选；评论气泡优先在目标空闲侧就近显示并跟随横纵滚动。候选改动作者脚本时会要求先审阅图表等动态内容。
