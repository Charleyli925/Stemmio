...
- 修复两个当前稿之间切换时同一 HTML 被初始打开、项目补充读取和静态资源基址连续重载造成的多次闪烁：补充读取复用首次 SourceReceipt，缓存交接只接受当前导航的精确回执，静态交接页只装载一次稳定文档。跨项目打开长期规则或历史现在只解析该页面的只读项目上下文，不再默认打开或短暂显示目标项目当前稿；页面保存、重试、历史浏览器打开及 AI 锁也按目标项目隔离。
