...
- 切换两个当前稿时，旧画布保持可见但不可操作，直到新画布核对正确后一次接替；打开失败保留明确的错误与重试入口。预览只按当前文档身份和内容判断可用，不再因评论、版本等后台工作闪灰。
- 等待切入的非活动标签增加轻量底色与细底线，并向辅助技术说明打开状态；打开结束后自动恢复普通标签外观，已选中标签内切换历史版本不改变标签名称。
